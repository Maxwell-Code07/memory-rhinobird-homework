...
现在使用 Bash 入口执行，默认写入新的 `results/` 子目录，不覆盖提交证据。该入口负责加载 `.env`、准备隔离的基础配置，再调用零依赖 Node.js soak 核心。
...
联合查看运行终端截图和 `conversations.jsonl` 中的错误记录。第 6 轮执行期间达到总时长，活动任务被取消，错误原因为 `duration_reached`，没有继续等待完整单轮超时。
...
当前 Shell 入口可使用 `bash ./run-basic-soak.sh --invalid-api-key` 复现同类容错验证；该命令预期返回非零退出码，并生成结构化 FAIL 结果。

![图 11：模型服务返回 HTTP 401 并归类为 authentication](./Figure_week3.1/11-invalid-api-key-http401.png)

### 图 12：0/1 通过与 FAIL 汇总

soak 退出码为 1，最终为 FAIL，终止原因 `consecutive_failures`。

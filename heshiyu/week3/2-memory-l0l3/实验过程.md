...
先确保记忆插件和 Gateway 已安装启动，再使用 Bash 入口自动执行富事实 soak 与 L0–L3、`/recall` 校验：
...
本阶段 12 张图只引用 `Figure_week3.2/`，均为运行过程中手动采集的 PNG。

### 图 01：Hermes 0.20.6 镜像构建记录

根据保留的 Docker 构建终端截图确认实际镜像标签、版本和完成输出。本目录 Dockerfile 与 Week2 交付物一致。

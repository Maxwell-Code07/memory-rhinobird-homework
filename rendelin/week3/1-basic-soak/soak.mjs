...
Options:
  ...
  --wait-after <seconds>          Sleep after the last round so the memory pipeline
                                  (L1 extraction / L2 scene / L3 persona) can flush
                                  before we report (default: 0 = no wait)
...
function parseArgsOrExit() {
  ...
  try {
    parsed = parseArgs({
      options: {
        ...
        "wait-after": { type: "string", default: "0" },
  ...
  try {
    return {
      ...
      waitAfter: parseIntArg(parsed.values["wait-after"], "wait-after"),
    };
  } catch (err) {
    console.error(`[soak] argument error: ${err.message}\n\n${USAGE}`);
    process.exit(2);
...
async function main() {
  ...
  // ---- Post-soak memory flush wait --------------------------------------
  // L1 extraction is deferred (l1IdleTimeout) and L2/L3 run off the L1 backlog
  // with their own min-interval delays; on a real memory-enabled Hermes the
  // four layers may not be on disk the instant the last round returns.  Give
  // the pipeline a grace period so a follow-up L0-L3 check (进阶 1) sees the
  // records / scene_blocks / persona.md that the conversation actually created.
  if (cfg.waitAfter > 0) {
    log(`waiting ${cfg.waitAfter}s for memory pipeline flush ...`);
    await sleepMs(cfg.waitAfter * 1000);
  }
  ...
  const verdict = {
    ...
    config: {
      ...
      wait_after_seconds: cfg.waitAfter,
    },
    rounds: {
